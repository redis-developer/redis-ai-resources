...
from .hierarchical_models import CourseSummary, CourseDetails
...
class HierarchicalContextAssembler:
    ...
    def _format_details(self, details: CourseDetails) -> str:
        ...
        # Schedule
        if details.schedule:
            days = ', '.join([d.value.title() for d in details.schedule.days])
            sections.append(
                f"**Schedule**: {days}, "
                f"{details.schedule.start_time.strftime('%I:%M %p')} - "
                f"{details.schedule.end_time.strftime('%I:%M %p')}"
            )
            if details.schedule.location:
                sections.append(f"**Location**: {details.schedule.location}")
        ...
        # Grading
        sections.append("\n### Grading Policy")
        grading = details.grading_policy
        if grading.homework_weight > 0:
            sections.append(f"- Homework: {grading.homework_weight * 100:.0f}%")
        if grading.project_weight > 0:
            sections.append(f"- Projects: {grading.project_weight * 100:.0f}%")
        if grading.quiz_weight > 0:
            sections.append(f"- Quizzes: {grading.quiz_weight * 100:.0f}%")
        if grading.midterm_weight > 0:
            sections.append(f"- Midterm: {grading.midterm_weight * 100:.0f}%")
        if grading.final_weight > 0:
            sections.append(f"- Final Exam: {grading.final_weight * 100:.0f}%")
        if grading.participation_weight > 0:
            sections.append(f"- Participation: {grading.participation_weight * 100:.0f}%")
        ...
        for assign_type, assignments in sorted(by_type.items(), key=lambda x: x[0].value):
        ...
        # Textbooks
        if details.textbooks:
            sections.append("\n### Textbooks")
            for book in details.textbooks:
                status = "Required" if book.required else "Recommended"
                sections.append(f"- [{status}] {book.title} by {', '.join(book.authors)}")
    ...
    def assemble_with_budget(
        ...
        if estimated > max_tokens and len(details) > 1:
            ...
            context = self.assemble_hierarchical_context(summaries, reduced_details, query)
            estimated = self.estimate_tokens(context)
...
class FlatContextAssembler:
    ...
    def assemble_flat_context(
        ...
        for i, course in enumerate(courses, 1):
            ...
            sections.append(f"Credits: {course.credits} | Level: {course.difficulty_level.value}")
...
class RawContextAssembler:
    ...
    def assemble_raw_context(
        ...
        data = {
            "query": query,
            "results": [course.model_dump() for course in courses]
        }
