...
class CourseIngestionPipeline:
    ...
    async def ingest_courses(self, courses_data: List[Dict[str, Any]]) -> int:
        ...
        with Progress() as progress:
            task = progress.add_task(
                "[green]Ingesting courses...", total=len(courses_data)
            )
            ...
            for course_data in courses_data:
                try:
                    ...
                    console.print(
                        f"[red]❌ Failed to ingest course {course_data.get('course_code', 'unknown')}: {e}[/red]"
                    )
    ...
    def ingest_majors(self, majors_data: List[Dict[str, Any]]) -> int:
        ...
        with Progress() as progress:
            task = progress.add_task(
                "[blue]Ingesting majors...", total=len(majors_data)
            )
            ...
            for major_data in majors_data:
                try:
                    ...
                    console.print(
                        f"[red]❌ Failed to ingest major {major_data.get('name', 'unknown')}: {e}[/red]"
                    )
    ...
    def verify_ingestion(self) -> Dict[str, int]:
        ...
        return {"courses": course_count, "majors": major_count}
    ...
    async def run_ingestion(self, catalog_file: str, clear_existing: bool = False):
        ...
        console.print(
            f"[blue]📊 Verification - Courses: {verification['courses']}, Majors: {verification['majors']}[/blue]"
        )
...
@click.command()
@click.option(
    "--catalog", "-c", default="course_catalog.json", help="Course catalog JSON file"
)
...
def main(catalog: str, clear: bool, redis_url: str):
    ...
    if not os.getenv("OPENAI_API_KEY"):
        ...
        console.print(
            "[yellow]Please set your OpenAI API key for embedding generation[/yellow]"
        )
        sys.exit(1)
