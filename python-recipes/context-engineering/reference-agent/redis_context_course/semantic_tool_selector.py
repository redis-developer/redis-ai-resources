...
class SemanticToolSelector:
    ...
    def __init__(self, tools: List[BaseTool], embeddings_model: Optional[OpenAIEmbeddings] = None):
        """
    ...
    def _initialize_tool_intents(self, tools: List[BaseTool]):
        ...
        tool_semantics = {
            "search_courses_tool": {
                ...
                "keywords": ["search", "find", "show", "discover", "browse", "list", "available"]
            },
            "get_recommendations_tool": {
                ...
                "keywords": ["recommend", "suggest", "should", "best", "plan", "path", "next"]
            },
            "store_preference_tool": {
                ...
                "keywords": ["prefer", "like", "remember", "save", "store", "want", "style"]
            },
            "store_goal_tool": {
                ...
                "keywords": ["goal", "want to become", "working toward", "aim", "target", "career"]
            },
            "get_student_context_tool": {
                ...
                "keywords": ["know about me", "my", "profile", "history", "preferences", "goals"]
            }
        }
        ...
        for tool in tools:
            ...
            if tool_name in tool_semantics:
                ...
                # Generate embedding
                try:
                    embedding = np.array(self.embeddings_model.embed_query(semantic_text))
        ...
        try:
            ...
            for tool_intent in self.tool_intents:
                if tool_intent.embedding is not None:
                    ...
                    keyword_boost = self._calculate_keyword_boost(query, tool_intent.keywords)
            ...
            selected_tools = [
                tool for tool, score, similarity in tool_scores 
                if similarity >= min_confidence
            ][:max_tools]
            
            # Log selection for debugging
            logger.info(f"Selected {len(selected_tools)} tools for query: '{query[:50]}...'")
            for tool, score, similarity in tool_scores[:max_tools]:
                logger.debug(f"  {tool.name}: similarity={similarity:.3f}, final_score={score:.3f}")
        ...
        try:
            ...
            for tool_intent in self.tool_intents:
                if tool_intent.embedding is not None:
                    ...
                    keyword_matches = [
                        kw for kw in tool_intent.keywords 
    ...
    def get_tool_coverage(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "tools_with_embeddings": sum(1 for ti in self.tool_intents if ti.embedding is not None),
            "tools": [
                {
                    "name": ti.tool.name,
