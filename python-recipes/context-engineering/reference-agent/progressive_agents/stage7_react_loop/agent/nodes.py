"""
Workflow nodes for the ReAct Loop Course Q&A Agent.

Stage 7: ReAct (Reasoning + Acting) loop with explicit reasoning traces.
"""
...
def get_agent_llm():
    ...
    if _agent_llm is None:
        from .tools import search_memories_tool, store_memory_tool

        llm = ChatOpenAI(model="gpt-4o", temperature=0.1, max_tokens=2000)
        # Bind all three tools: search_courses, search_memories, store_memory
        _agent_llm = llm.bind_tools(
            [search_courses_tool, search_memories_tool, store_memory_tool]
        )
    return _agent_llm
...
# ============================================================================
# NEW: ReAct Agent Node (Stage 7)
# ============================================================================

# Import ReAct agent from separate module
from .react_agent import react_agent_node

# Alias for compatibility with workflow
agent_node = react_agent_node


# ============================================================================
# OLD: Iterative Tool Calling Agent (Stage 6) - REPLACED BY REACT
# ============================================================================
# This is kept for reference but not used in Stage 7


async def agent_node_stage6_iterative(state: WorkflowState) -> WorkflowState:
    """
    OLD Stage 6 agent with iterative tool calling (implicit reasoning).

    This has been REPLACED by react_agent_node which uses explicit reasoning.
    ...
    # Set global student_id for memory tools to access
    from . import tools
...
1. **search_courses**: Search the Redis University course catalog using semantic search, exact course code matching,
   ...
   When using search_courses, determine:
   - **intent**: GENERAL, PREREQUISITES, SYLLABUS_OBJECTIVES, or ASSIGNMENTS
   - **search_strategy**: "exact_match" (for course codes like CS004), "hybrid" (best for most queries), or "semantic_only"
   - **course_codes**: Extract any specific course codes mentioned (e.g., ["CS004"])
   - **information_type**: What specific info is needed (e.g., ["prerequisites"], ["syllabus"])
   - **departments**: Filter by department if mentioned (e.g., ["Computer Science"])
...
2. **search_memories**: Search the student's long-term memory for their preferences, goals, past interactions, and
   ...
   - The query might benefit from knowing student preferences or past interactions
   - You want to personalize course recommendations
   - The student asks about their own preferences or goals
...
3. **store_memory**: Store important information about the student to long-term memory for future sessions.
   ...
   Use store_memory when students share:
   ...
   - Interests: "I'm interested in machine learning"
...
Examples:
- "I'm interested in ML. I prefer online courses" → store_memory (2x), then search_courses
- "What courses would you recommend?" → search_memories first, then search_courses with personalization
- "What is CS004?" → search_courses with exact_match strategy
- "I want to work in AI research" → store_memory, then search_courses for AI courses"""
        ...
        # Agentic loop: Allow LLM to call tools iteratively
        max_iterations = 5  # Prevent infinite loops
        iteration = 0

        logger.info(f"   🧠 Starting agentic loop (max {max_iterations} iterations)...")
            ...
            if response.tool_calls:
                ...
                for tool_call in response.tool_calls:
                    ...
                    if tool_name == "search_courses":
                        ...
                        tool_result = await search_memories_tool.ainvoke(tool_args)
                    elif tool_name == "store_memory":
                        from .tools import store_memory_tool

                        tool_result = await store_memory_tool.ainvoke(tool_args)
                    else:
                ...
                # Continue loop - LLM can decide to call more tools or finish
                continue

            else:
                # No tool calls - LLM is done
                logger.info(
                    f"      ✅ LLM finished (no more tool calls after {iteration} iteration(s))"
                )
