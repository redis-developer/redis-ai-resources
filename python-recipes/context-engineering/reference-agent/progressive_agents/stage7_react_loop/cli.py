#!/usr/bin/env python3
"""
Interactive CLI for the ReAct Loop Course Q&A Agent (Stage 7).
...
Usage:
    ...
    python cli.py --student-id alice --show-reasoning          # Show reasoning traces
"""
...
class ReActCLI:
    """Interactive CLI for ReAct Loop Course Q&A Agent (Stage 7)."""
    ...
    async def initialize(self):
        ...
        print("=" * 80)
        print("ReAct Loop Course Q&A Agent - Stage 7")
        print("=" * 80)
        print()
        ...
        try:
            ...
            print("🔧 Initializing Long-term Memory Course Q&A Agent (Stage 6)...")
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Stage 7 ReAct Loop Agent - Explicit Reasoning with Thought → Action → Observation"
    )
    parser.add_argument(
        "query",
        nargs="?",
    ...
    parser.add_argument(
        "--show-reasoning",
        action="store_true",
        help="Display explicit reasoning traces (Thought → Action → Observation)",
    )
