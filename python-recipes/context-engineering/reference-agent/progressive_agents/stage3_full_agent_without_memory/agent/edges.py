...
def route_after_cache_check(state: Dict[str, Any]) -> Literal["research", "synthesize"]:
    ...
    # Count cache misses
    cache_misses = [
        question for question, is_cached in cache_hits.items() if not is_cached
    ]
...
def route_after_quality_evaluation(
    ...
    if needs_improvement:
        logger.info(
            f"🔄 Routing to additional research: {len(needs_improvement)} questions need improvement"
        )
        for question, score, iteration in needs_improvement:
            logger.info(
                f"   🔍 Improve: '{question[:40]}...' (score: {score:.2f}, iteration: {iteration + 1})"
            )
        return "research"
    else:
