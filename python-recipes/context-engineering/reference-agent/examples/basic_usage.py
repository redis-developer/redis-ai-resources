...
def demo_models():
    ...
    print(
        f"  Preferences: {student.preferred_format.value}, {student.preferred_difficulty.value}"
    )
...
def demo_package_info():
    ...
    for name, description in components:
        available = (
            "✅" if getattr(redis_context_course, name, None) is not None else "❌"
        )
...
def main():
    ...
    try:
        ...
        print("\nNext steps:")
        print(
            "1. Install Redis 8: docker run -d --name redis -p 6379:6379 redis:8-alpine"
        )
