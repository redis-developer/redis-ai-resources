...
class SystemHealthChecker:
    ...
    async def check_infrastructure(self) -> List[CheckResult]:
        ...
        for var, description in env_vars.items():
            ...
            if not value or value == 'your_openai_api_key_here':
                ...
                display_value = value[:8] + '...' + value[-4:] if 'API_KEY' in var else value
                results.append(CheckResult(
                    name=f"Environment: {var}",
                    status=CheckStatus.PASS,
    ...
    def detect_data_patterns(self) -> Dict[str, List[str]]:
        ...
        patterns = {
            ...
            'other': [k for k in all_keys if not any(p in k.lower() for p in ['major', 'course', 'memory'])]
        }
        ...
        if course_count > 0:
            ...
            if patterns['courses']:
                ...
                try:
                    ...
                    required_fields = ['course_code', 'title', 'description', 'content_vector']
                    missing_fields = [f for f in required_fields if f not in field_names]

                    if not missing_fields:
                        # Get text fields safely
                        course_code = sample_data.get(b'course_code', b'N/A').decode('utf-8')
    ...
    def generate_summary(self) -> Dict[str, any]:
        ...
        return {
            ...
            'warnings': warnings,
            'failed': failed,
            'critical_issues': [r for r in self.results if r.status == CheckStatus.FAIL],
            'avg_performance': sum(r.performance_ms for r in self.results if r.performance_ms) / max(1, len([r for r in self.results if r.performance_ms]))
        }
...
{"-" * 20}""")
        ...
        if not any(r.status == CheckStatus.FAIL and "Redis Connection" in r.name for r in infra_results):
...
async def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Redis Context Course System Health Check")
